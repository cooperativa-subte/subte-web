...
export default function Projects(props) {
  ...
  const [filter] = useState(() =>
    qs.parse(props.location.search.slice(1)).tag != null
      ? qs.parse(props.location.search.slice(1)).tag
      : '',
  );
  ...
  useEffect(() => {
    getProjects().then((projects) => {
      ...
      if (filter) {
        ...
        setFilteredProjects(shuffleArray(projects));
      }
    });
  }, [filter]);
              ...
              {tags.length > 0 &&
                tags.map((tag, i) => (
                  <span
                    ...
                    className={
                      activeTag === tag
                        ? 'active tag-button float-left'
                        : 'tag-button float-left'
                    }
                  >
                    {tag}
                  </span>
            ...
            {filteredProjects.length > 0 &&
              filteredProjects.map((project, i) => {
                return (
                  <div
                    key={project.id}
                    className={`col-12 col-md-4 project-card-container text-left mt-4`}
                  >
